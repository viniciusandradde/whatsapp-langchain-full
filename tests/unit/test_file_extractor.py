...
def test_extract_txt_utf8():
    text = extract_text("notas.txt", "Olá mundo, café.".encode("utf-8"))
    assert "Olá mundo" in text
    assert "café" in text


def test_extract_md_preserves_markdown():
    md = "# Título\n\n- item 1\n- item 2"
    text = extract_text("doc.md", md.encode("utf-8"))
...
def test_extract_docx_returns_paragraphs():
    ...
    text = extract_text("politica.docx", raw)
...
def test_extract_pdf_returns_text():
    ...
    text = extract_text("policy.pdf", raw)
...
def test_extract_pdf_invalid_raises():
    with pytest.raises(FileExtractionError, match="PDF inválido"):
        extract_text("falso.pdf", b"not a pdf at all")
...
def test_extract_too_large_raises():
    ...
    with pytest.raises(FileTooLargeError):
        extract_text("big.txt", huge)


def test_extract_truncates_at_max_text_chars(monkeypatch):
    ...
    text = extract_text("a.txt", ("x" * 500).encode())
    assert len(text) == 100


def test_extract_unsupported_extension_raises():
    with pytest.raises(UnsupportedFileTypeError):
        extract_text("img.png", b"fake png bytes")
