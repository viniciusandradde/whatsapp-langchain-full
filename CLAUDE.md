...
- `133_whitelist_numero.sql` — whitelist de números que a IA responde mesmo com conexão em manual (contatos pessoais); `134_plano_pessoal.sql` — plano billing "Pessoal" R$97; `135_resumo_diario.sql` — resumo diário de atendimentos por WhatsApp (cols `resumo_diario_*` em `empresa`; worker `shared/resumo_diario.py`, claim atômico 1x/dia)
