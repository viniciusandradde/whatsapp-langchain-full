...
**Migrations** — application schema lives in `db/migrations/*.sql` (controlled by `_migrations` table; lock id `8_642_000`). LangGraph schema (`checkpoints*`, `store*`) is created in-code by `bootstrap_langgraph_schema()` at startup. Don't write SQL migrations for LangGraph tables. **Currently 175 migration files** numbered up to `181` (chronological gaps existem — não são problema; o número do arquivo ≠ contagem). Recent highlights:
