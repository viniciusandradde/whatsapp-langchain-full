...
Campos principais:
- `message_id`: id externo da mensagem no provider. **Múltiplas rows com mesmo `message_id`:** ocorre quando um webhook traz mais de uma mídia. Cada mídia vira uma row independente; ordenação por `created_at`.
