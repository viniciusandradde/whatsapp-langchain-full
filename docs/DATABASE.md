...
Campos principais:
- `message_id`: id externo do provider (ex: `data.key.id` na Evolution). **Múltiplas rows com mesmo `message_id`:** ocorre quando o webhook traz NumMedia > 1. Cada mídia vira uma row independente; ordenação por `created_at`.
