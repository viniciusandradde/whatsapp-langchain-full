...
| A | Limites existentes + `limite_agentes` + atendimentos/mês suave + teto de IA no `ia_budget` | `feat/plano-leva-a-limites` | ⏳ em andamento | | mig 189; `test_plano_leva_a_endpoints.py` |
| A.0 | Consulta de uso em produção + flags de grandfathering (inclui `plano.limite_orcamento_ia_usd` da 1018) | — | ⏳ | | saída da consulta anexada na PR |
