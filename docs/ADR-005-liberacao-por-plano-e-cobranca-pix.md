...
| C2 | Quantidades e retenção (departamentos, workflows, menus, retenção, auditoria, csat, resumo, bateria, observabilidade, qualidade) | `feat/plano-leva-c2-quantidades` | 🔍 no dev, aguardando o dono | 20/09/2026 | mig 192; `test_plano_leva_c2_endpoints.py` (13 verdes); mensagens de 402 reescritas para usuário final (regra do dono) |
...
| E | Vigência: `plano_valido_ate`, avisos D-7/D-3/D0 (cliente + superadmin), carência 5 dias, rebaixamento automático, colunas genéricas em `transacao` | `feat/plano-leva-e-vigencia` | ⬜ | | mig 193 |
| E.1 | Texto dos avisos de vencimento aprovado pelo dono | — | ⬜ | | |
| F | Cobrança por planos hospedados: links por plano, "Registrar pagamento" (superadmin), `/billing` com links + vigência + histórico | `feat/billing-planos-hospedados` | ⬜ | | mig 194; substitui as antigas F/G/H (decisão do dono 20/09) |
| F.0 | Dono cria os planos na InfinitePay (Cobrança Recorrente — padrão) e no Mercado Pago (Assinaturas — alternativa), passa os links, confere as taxas de cartão dos dois e fixa o preço do Pro (D10) | — | ⬜ dono | | |
