...
| D | Painel: `usePlano`, cadeados no menu e nas telas, `/billing` com as chaves reais | `feat/plano-leva-d-painel` / **#170** | 🔍 no dev, aguardando o dono | 21/09/2026 | sem migração; `GET /api/empresas/{id}/plano`; cadeados no menu (sidebar + ⌘K) e em 10 interruptores; `/billing` com tabela pelas chaves reais e destaque de `?feature=`; `test_plano_leva_d_endpoints.py` (4 E2E verdes); capturas claro/escuro 1440/390 |
