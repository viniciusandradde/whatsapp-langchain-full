...
| F.0.1 | Links da InfinitePay migrados pelo dono para a conta `vsa-tecnologia` (Pessoal com id novo `y3lpTMjINF`; Pro/Enterprise mesmo id) — aplicados em produção e no dev por `PUT /api/billing/planos/{slug}/links` (MP preservado); **mig 195** corrige o seed para instalação nova (só substitui o prefixo antigo; link posto à mão fica) | `fix/plano-links-infinitepay-seed` / **#173** (`775dc34`) | ✅ em produção | 21/09/2026 | api/worker-1/2 recriados 09:55 UTC com a 195 na imagem; `_migrations` = 189, links inalterados (no-op). A PR também pôs `db/**` no filtro `backend` do `deploy.yml` — sem isso uma PR só de migration passava verde sem reconstruir a api (que aplica as migrations do `db/` da própria imagem). Validação no dev: transação revertida (seed antigo → trocado; link à mão → intocado) |
