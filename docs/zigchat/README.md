...
GrupoSistema   { id, nome, descricao, permissoes[], empresa_id }   # = "perfil"
...
3. **Permissão no usuário, além do grupo** — `Usuario.permissoes[]` permite exceção individual sem criar um perfil novo.

E a **segmentação de acesso** não é só permissão: o usuário tem `departamentos[]`, `conexoes[]` e `canais[]` — ele só enxerga o que está vinculado a ele.
...
| **Escopo por conexão/canal** | **"Visualizar Todas as Conexões" + `Usuario.conexoes[]`** | **não existe** |
| Permissão direta no usuário | sim | não (só via perfil) |
