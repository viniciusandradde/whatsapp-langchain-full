...
_PATTERNS_RAW: list[tuple[str, str]] = [
    ...
    (
        "jailbreak_en",
        r"\b(ignore|disregard|forget)\s+(all\s+)?(previous|prior|above)\s+(instructions|prompts?|rules?)",
    ),
    ("jailbreak_en2", r"\b(DAN|do\s*anything\s*now|jailbroken)\b"),
    (
        "override_en",
        r"\byou\s+are\s+(now|actually)\s+(an?\s+)?(unrestricted|uncensored|no\s+restrictions)",
    ),
    (
        "system_extract_en",
        r"\b(reveal|show|tell\s+me|print|output)\s+(your|the)\s+(system\s+)?(prompt|instructions|rules)\b",
    ),
    # Jailbreak PT-BR
    (
        "jailbreak_pt",
        r"\b(ignore|esque[çc]a|desconsidere)\s+(tudo|todas?|as)\s+(instru[çc][õo]es|regras|prompts?)",
    ),
    ("override_pt", r"\b(agora|a\s+partir\s+de\s+agora)\s+voc[êe]\s+[ée]\s+"),
    (
        "system_extract_pt",
        r"\b(mostre?|revele?|imprima)\s+(seu|o)\s+(prompt|instru[çc][õo]es|sistema)",
    ),
    ("role_swap_pt", r"\b(finja|simule|assuma)\s+(que|ser)\s+"),
    # Pedidos diretos de violação
    (
        "desconto_ilicito",
        r"\b(me\s+d[êe]|d[êe]\s+me)\s+(\d+\s*%?\s*)?desconto\s+(de\s+)?(\d+\s*%|gr[áa]tis|grand[ée])",
    ),
    ...
    (
        "sql_inject",
        r"\b(DROP|DELETE|TRUNCATE)\s+(TABLE|FROM|DATABASE)\b",
    ),
]
...
def check_input(text: str) -> InputFilterResult:
    ...
    for name, regex in _COMPILED:
        ...
        if m:
            ...
            return InputFilterResult(blocked=True, pattern=name, sample=sample[:100])
    return InputFilterResult(blocked=False, pattern=None, sample=None)
