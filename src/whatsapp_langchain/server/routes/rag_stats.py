...
@router.post("/preview", response_model=list[PreviewHit])
async def preview_search(
    body: PreviewRequest,
    empresa_id: int = Depends(get_empresa_context),
) -> list[PreviewHit]:
    """Playground: simula a busca RAG sem chamar o agente.

    Útil pra admin testar query → ver quais chunks subiriam, ajustar
    docs ou pastas pra melhorar relevância.
    """
    ...
    try:
        results = await search_relevant(
            pool, empresa_id, body.query, pasta_ids=body.pasta_ids
        )
